#delimit;																								
		...
		replace	country	=	"Antigua & Barbuda"	if	cowcode	==	58	;														
		...
		replace	country	=	"Austria-Hungary" 	if	cowcode	==	300	;														
		replace	country	=	"Azerbaijan"	if	cowcode	==	373	;														
		replace	country	=	"Baden"	if	cowcode	==	267	;														
		replace	country	=	"Bahamas, The"	if	cowcode	==	31	;														
		replace	country	=	"Bahrain"	if	cowcode	==	692	;														
		replace	country	=	"Bangladesh" 	if	cowcode	==	771	;														
		replace	country	=	"Barbados" 	if	cowcode	==	53	;														
		replace	country	=	"Bavaria" 	if	cowcode	==	245	;														
		...
		replace	country	=	"Bosnia and Herzegovina"	if	cowcode	==	346	;														
		replace	country	=	"Botswana"	if	cowcode	==	571	;														
		replace	country	=	"Brazil"	if	cowcode	==	140	;														
		replace	country	=	"Brunei Darussalam"	if	cowcode	==	835	;														
		...
		replace	country	=	"Congo, Rep." 	if	cowcode	==	484	;														
		...
		replace	country	=	"Timor-Leste"	if	cowcode	==	860	;														
		replace	country	=	"Ecuador"	if	cowcode	==	130	;														
		replace	country	=	"Egypt, Arab Rep."	if	cowcode	==	651	;														
		...
		replace	country	=	"Micronesia, Fed. Sts."	if	cowcode	==	987	;														
		...
		replace	country	=	"Gambia, The"	if	cowcode	==	420	;														
		replace	country	=	"Georgia"	if	cowcode	==	372	;														
		replace	country	=	"Germany, East"	if	cowcode	==	265	;														
		replace	country	=	"Germany, West"	if	cowcode	==	260	;														
		...
		replace	country	=	"Guinea-Bissau"	if	cowcode	==	404	;														
		replace	country	=	"Guyana"	if	cowcode	==	110	;														
		replace	country	=	"Haiti"	if	cowcode	==	41	;														
		replace	country	=	"Hanover"	if	cowcode	==	240	;														
		replace	country	=	"Hesse Electoral"	if	cowcode	==	273	;														
		replace	country	=	"Hesse Grand Ducal"	if	cowcode	==	275	;														
		...
		replace	country	=	"Iran, Islamic Rep."	if	cowcode	==	630	;														
		...
		replace	country	=	"Cote d'Ivoire"	if	cowcode	==	437	;														
		...
		replace	country	=	"Korea, North"	if	cowcode	==	731	;														
		...
		replace	country	=	"Sao Tome and Principe"	if	cowcode	==	403	;														
		replace	country	=	"Saudi Arabia"	if	cowcode	==	670	;														
		replace	country	=	"Saxony"	if	cowcode	==	269	;														
		...
		replace	country	=	"Korea, South"	if	cowcode	==	732	;														
		...
		replace	country	=	"St. Vincent and the Grenadines"	if	cowcode	==	57	;														
		...
		replace	country	=	"Syrian Arab Republic"	if	cowcode	==	652	;														
		...
		replace	country	=	"Trinidad and Tobago"	if	cowcode	==	52	;														
		replace	country	=	"Tunisia"	if	cowcode	==	616	;														
		replace	country	=	"Turkey"	if	cowcode	==	640	;														
		replace	country	=	"Turkmenistan"	if	cowcode	==	701	;														
		replace	country	=	"Tuscany"	if	cowcode	==	337	;														
		replace	country	=	"Tuvalu"	if	cowcode	==	947	;														
		replace	country	=	"Two Siciles"	if	cowcode	==	329	;														
		replace	country	=	"Russian Federation"	if	cowcode	==	365	;														
		...
		replace	country	=	"United Kingdom"	if	cowcode	==	200	;														
		replace	country	=	"United States"	if	cowcode	==	2	;														
		...
		replace	country	=	"Wuerttemburg"	if	cowcode	==	271	;														
		...
		replace	country	=	"Yugoslavia / Serbia & Montenegro"	if	cowcode	==	345	;														
		replace	country	=	"Zaire / Dem. Rep. Comgo"	if	cowcode	==	490	;														
		replace	country	=	"Zambia"	if	cowcode	==	551	;														
		replace	country	=	"Zanzibar"	if	cowcode	==	511	;														
		replace	country	=	"Zimbabwe"	if	cowcode	==	552	;														
		replace	country	=	"Hong Kong, SAR"	if	cowcode	==	715	;														
		replace	country	=	"Macao, SAR"	if	cowcode	==	716	;														
																								
																								
																								
		/*WDI countries */																						
		replace	country	=	"American Samoa"	if	cowcode	==	1001	;														
		replace	country	=	"Aruba"	if	cowcode	==	1002	;														
		replace	country	=	"Bermuda"	if	cowcode	==	1003	;														
		replace	country	=	"Cayman Islands"	if	cowcode	==	1004	;														
		replace	country	=	"Channel Islands"	if	cowcode	==	1005	;														
		replace	country	=	"Curacao"	if	cowcode	==	1006	;														
		replace	country	=	"Faeroe Islands"	if	cowcode	==	1007	;														
		replace	country	=	"French Polynesia"	if	cowcode	==	1008	;														
		replace	country	=	"Greenland"	if	cowcode	==	1009	;														
		replace	country	=	"Guam"	if	cowcode	==	1010	;														
		replace	country	=	"Isle of Man"	if	cowcode	==	1011	;														
		replace	country	=	"New Caledonia"	if	cowcode	==	1012	;														
		replace	country	=	"Northern Mariana Islands"	if	cowcode	==	1013	;														
		replace	country	=	"Puerto Rico"	if	cowcode	==	1014	;														
		replace	country	=	"Serbia"	if	cowcode	==	1015	;														
		replace	country	=	"Sint Maarten (Dutch part)"	if	cowcode	==	1016	;														
		replace	country	=	"St. Martin (French part)"	if	cowcode	==	1017	;														
		replace	country	=	"Turks and Caicos Islands"	if	cowcode	==	1018	;														
		replace	country	=	"Virgin Islands (U.S.)"	if	cowcode	==	1019	;														
		replace	country	=	"West Bank and Gaza"	if	cowcode	==	1020	;														
		replace	country	=	"South Sudan"	if	cowcode	==	1021	;														
																								
																								
																								
		/*WDI geographic areas */																						
		replace	country	=	"East Asia & Pacific (all income levels)"	if	cowcode	==	2010	;														
		replace	country	=	"East Asia & Pacific (developing countries only)"	if	cowcode	==	2011	;														
		replace	country	=	"Europe & Central Asia (all income levels)"	if	cowcode	==	2020	;														
		replace	country	=	"Europe & Central Asia (developing countries only)"	if	cowcode	==	2021	;														
		replace	country	=	"Latin America & Caribbean (all income levels)"	if	cowcode	==	2030	;														
		replace	country	=	"Latin America & Caribbean (developing countries only)"	if	cowcode	==	2031	;														
		replace	country	=	"Middle East & North Africa (all income levels)"	if	cowcode	==	2040	;														
		replace	country	=	"Middle East & North Africa (developing countries only)"	if	cowcode	==	2041	;														
		replace	country	=	"Sub-Saharan Africa (all income levels)"	if	cowcode	==	2050	;														
		replace	country	=	"Sub-Saharan Africa (developing countries only)"	if	cowcode	==	2051	;														
		replace	country	=	"North America"	if	cowcode	==	2060	;														
		replace	country	=	"European Union"	if	cowcode	==	2070	;														
		replace	country	=	"Euro area"	if	cowcode	==	2071	;														
		replace	country	=	"OECD members"	if	cowcode	==	2072	;														
		replace	country	=	"Arab World"	if	cowcode	==	2080	;														
		replace	country	=	"South Asia"	if	cowcode	==	2085	;														
		replace	country	=	"Small states"	if	cowcode	==	2090	;														
		replace	country	=	"Pacific island small states"	if	cowcode	==	2091	;														
		replace	country	=	"Caribbean small states"	if	cowcode	==	2092	;														
		replace	country	=	"Other small states"	if	cowcode	==	2093	;														
		replace	country	=	"World"	if	cowcode	==	2099	;														
		replace	country	=	"Not classified"	if	cowcode	==	2100	;														
																								
																								
		/*WDI income groups*/																						
		replace	country	=	"Heavily indebted poor countries (HIPC)"	if	cowcode	==	3001	;														
		replace	country	=	"Least developed countries"	if	cowcode	==	3002	;														
		replace	country	=	"Low income"	if	cowcode	==	3003	;														
		replace	country	=	"Lower middle income"	if	cowcode	==	3004	;														
		replace	country	=	"Low & middle income"	if	cowcode	==	3005	;														
		replace	country	=	"Middle income"	if	cowcode	==	3006	;														
		replace	country	=	"Upper middle income"	if	cowcode	==	3007	;														
		replace	country	=	"High income"	if	cowcode	==	3008	;														
		replace	country	=	"High income: OECD"	if	cowcode	==	3009	;														
		replace	country	=	"High income: nonOECD"	if	cowcode	==	3010	;														
																								
		/*AMECO aggregates*/																						
		replace	country	=	"EA12 (12 countries)"	if	cowcode	==	2101	;														
		replace	country	=	"EA12 (including D_W West-Germany)"	if	cowcode	==	2102	;														
		replace	country	=	"EA17 (17 countries)"	if	cowcode	==	2103	;														
		replace	country	=	"EU12 (12 New Member States)"	if	cowcode	==	2104	;														
		replace	country	=	"EU15 (15 countries)"	if	cowcode	==	2105	;														
		replace	country	=	"EU15 (including D_W West Germany)"	if	cowcode	==	2106	;														
		replace	country	=	"EU27 (27 countries)"	if	cowcode	==	2107	;														
		replace	country	=	"Euro area (12 countries)"	if	cowcode	==	2108	;														
		replace	country	=	"Euro area (17 countries)"	if	cowcode	==	2109	;														
		replace	country	=	"European Union (15 countries)"	if	cowcode	==	2110	;														
		replace	country	=	"European Union (27 countries)"	if	cowcode	==	2111	;														
		replace	country	=	"European Union (28 countries)"	if	cowcode	==	2112	;														
		replace	country	=	"linked Germany)"	if	cowcode	==	255	;														
																								
		/*GSF additions*/																						
		replace	country	=	"Anguilla"	if	cowcode	==	1022	;														
		replace	country	=	"Netherlands Antilles"	if	cowcode	==	1023	;														
